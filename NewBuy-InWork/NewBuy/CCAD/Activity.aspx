<%@ Page Title="CCAD Activity" Language="C#" MasterPageFile="~/NewBuy.Master" AutoEventWireup="true" CodeBehind="Activity.aspx.cs" Inherits="NewBuy.CCAD.Activity" %>
    ...
    <script type="text/javascript">
        ...
        jQuery(function () {
                ...
                buttons: {
                    "Submit": function () {
                        ...
                        if (ordJustificationIds.length > 0) {
                            ...
                            //Send ajax request - update asset_manager_remarks field in req table
                            $.ajax({
            ...
            $("#lookup-comments").dialog({
                ...
                open: function (event) {
                    ...
                    //add additional button if not req grid
                    if (getCurrentTabIndex() != 0) {
                        CommentBtns["Save & Close"] = function () {
                            //Save row
                            $('#amRemarkSave').click();
                    ...
                    //set dialog buttons to dialog
                    $(this).dialog('option', 'buttons', CommentBtns);
                }
            });
        ...
        $(function () {
            ...
            $("#listReq").jqGrid({
                ...
                datatype: 'json',
                mtype: 'POST',
                postData: { SpoUser: "", Part_no: ""}, //Add default custom post parameters
                serializeGridData: function (postData) {
                    //Get asset manager
                    postData.SpoUser = jQuery('#reqButton1').prop('value');
                ...
                loadError: function (xhr, status, error) {
                    ...
                    ErrorHandling(xhr, "requirements grid");
                },
                gridComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        btnComments = "<button id='comment_" + ids[i] + "' type='button' class='gridButton' title='View Comment' value='" + ids[i] + "'></button>";

                        //Add buttons to jqGrid
                        jQuery("#listReq").jqGrid('setRowData', ids[i], { actions: btnGoToOrder, comments: btnComments });
                        ...
                        //View Comment Button
                        $('#' + ids[i] + ' :button[id^="comment_"]').button({
                            icons: {
                        ...
                        .click(function () {
                            ...
                            //Hide AM and CCB Remark boxes for Requirments Tab Comments Dialog
                            $("#amRemarkGridWrapper").hide();
                            $("#ccbRemarkGridWrapper").hide();

                            //Show SPO Remark boxes for Requirments Tab Comments Dialog
                            $("#spoRemarkGridWrapper").show();
                ...
                colModel: [
                            ...
                            { name: 'status', label: 'Overall Status', sorttype: 'text', align: 'left', width: 310 },
                            { name: 'change_reason', label: 'Change Reason', sorttype: 'text', align: 'left' },
                            { name: 'spo_request_id', label: 'Request ID', sorttype: 'int', align: 'center' },
                            { name: 'spo_export_date', label: 'Export Date', sorttype: 'date', datefmt: 'm/d/Y', align: 'right' },
                            { name: 'request_due_date', label: 'Due Date', sorttype: 'date', datefmt: 'm/d/Y', align: 'right' },
                            {
                                name: 'item_cost', label: 'Cost', sorttype: 'currency', align: 'right', formatter: 'currency',
                                formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2 }
                            },
                            { name: 'site', label: 'Site', sorttype: 'text', align: 'right' },
                            { name: 'comments', label: 'Comments', sortable: false, align: 'center' },
                            // PRP 05/31/16: added for new fields requested
                            // MIN_BUY_QTY
                            // SUPPLIER_MONTHLY_CAPACITY_QTY
                            // ANNUAL_BUY_IND
                            { name: 'order_quantity', label: 'Order Total', sorttype: 'text', align: 'left', width: 50 },
                ...
                ondblClickRow: function (rowid, iRow, iCol) {
                    ...
                    for (var i = 0; i < selectedReqRows.length; i++) {
                        if (selectedReqRows[i] == rowid) {
                            jQuery('#listReq').jqGrid('setSelection', rowid, false);
                        }
                    }

                    jQuery('#listReq').jqGrid('setSelection', rowid);
        ...
        /* Orders Grid */
        var ordMultiSel;
        var ordJustificationIds;  //orders change reason check
        ...
        $(function () {
            ...
            $("#listOrder").jqGrid({
                        ...
                        $('#' + ord_ids[i] + ' :button[id^="comment_"]').button({
                            ...
                            .click(function () {
                                ...
                                if (remarkGridID == "") {
                                    ...
                                    //Hide SPO Remarks box for Orders Tab Comments Dialog
                                    $("#spoRemarkGridWrapper").hide();

                                    //Show AM and CCB Remarks boxes for Orders Tab Comments Dialog
                                    $("#amRemarkGridWrapper").show();
                                    $("#ccbRemarkGridWrapper").show();
                ...
                colModel: [
                                ...
                                { name: 'spo_request_id', hidden: true },
                                ...
                                { name: 'order_quantity', label: 'Order Quantity', sorttype: 'int', align: 'center', editable: true, editrules: { required: true, integer: true, minValue: 1} },
                                ...
                                { name: 'change_reason', label: 'Change Reason', sorttype: 'text', align: 'center',
                                    editable: true, edittype: 'select', editrules: { required: false },
                                    editoptions: { value: '<%= getChangeReasons() %>' }
                                },
                                { name: 'order_no', label: 'Order No', sorttype: 'text', align: 'center' },
                ...
                grouping: true,
                groupingView: {
                    groupField: ['spo_request_id'],
                    groupColumnShow: [false],
                    groupText: ['SPO Req: <b>{0}</b>']
                },
                ondblClickRow: function (rowid, iRow, iCol) {
                ...
                serializeRowData: function (postData) {
                    ...
                    if (postData.id == "new_row") {
                        ...
                        var sel_req_id = $selectedReqGrid.jqGrid('getGridParam', 'selrow');
                        postData.internal_order_no = parseInt(sel_req_id);

                        //Get part_no from requirement grid
                        var sel_req_partnum = $selectedReqGrid.jqGrid('getCell', sel_req_id, 'part_no');
                        postData.part_no = sel_req_partnum;
                    }

                    //add requirement_schedule_no/ part_no for non-new records
                    if (postData.oper != "new") {
                        ...
                        postData.part_no = rowdata.part_no;
            ...
            jQuery("#listOrder").jqGrid('navGrid', '#pagerOrd',
                ...
                onClickButton: function () {
                    ...
                    if (currentlyGrouped) {
                        ...
                        //Add grouping
                        $ordGrid.jqGrid('groupingGroupBy', 'spo_request_id');
                    }
                ...
                onClickButton: function () {
                    //Remove button highlight
                    jQuery('#addOrderToGrid').removeClass("ui-state-active ui-state-hover ui-state-focus");
                    ...
                    if (selectedReqRowIDs.length > 0) {
                        ...
                        $.each(selectedReqRowIDs, function (index, value) {
                            ...
                            { part_no: reqSelRowData.part_no,
                                spo_request_id: reqSelRowData.spo_request_id,
                    ...
                    if (selectedOrderRows.length > 0) {
                        ...
                        $.ajax({
                            ...
                            error: function (jqXHR, status, error) {
                                ...
                                //Show error dialog
                                ErrorHandling(jqXHR, "validate ccn");
                                return false;
                            }
                        });
                    }
                    ...
                    if (selectedOrderRows.length > 0) {
                        ...
                        $.ajax({
                            ...
                            error: function (jqXHR, status, error) {
                                ...
                                //Show error dialog
                                ErrorHandling(jqXHR, "approve orders");
                                return false;
                            }
                        });
        ...
        $(function () {
            ...
            $("#listManager").jqGrid({
                ...
                datatype: 'json',
                mtype: 'POST',
                postData: { AssetManager: "" }, //Add default custom post parameters
                serializeGridData: function (postData) {
                    //Get asset manager
                ...
                colModel: [
                            ...
                            { name: 'asset_manager_user_name', label: 'Asset Manager', sortable: false, sorttype: 'text', align: 'left' }
                          ],
                jsonReader: {
                    root: "rows",
                    page: "currentPage",
        ...
        $(function () {
            ...
            $("#listPart").jqGrid({
                ...
                datatype: 'json',
                mtype: 'POST',
                postData: { Part_no: "", LtaPart: "false" }, //Add default custom post parameters
                serializeGridData: function (postData) {
                    //get part number
                    if (flagLtaPartGrid) {
        ...
        $(function () {
            ...
            $("#listVariance").jqGrid({
                ...
                loadError: function (xhr, status, error) {
                    ...
                    ErrorHandling(xhr, "New Buy variance comment");
                },
                loadui: 'block', //Loading message options
                autowidth: true,
                height: '100%',
        ...
        $(function () {
            ...
            $("#listAmRemark").jqGrid({
                ...
                loadError: function (xhr, status, error) {
                    ...
                    ErrorHandling(xhr, "asset manager remark grid");
                },
                loadui: 'block', //Loading message options
                autowidth: true,
                height: '100%',
        ...
        $(function () {
            //Create jqgrid, set options
            $("#listSpoRemark").jqGrid({
                url: "Activity.aspx/getSpoRemarkGrid",
                datatype: 'json',
                mtype: 'POST',
                postData: { InternalOrderNo: "0" }, //Add default custom post parameters
                serializeGridData: function (postData) {
                    //Get order number
                    if (remarkGridID) {
                        postData.InternalOrderNo = remarkGridID;
                    }
                ...
                loadComplete: function (data) {
                    var mygrid = jQuery("#listSpoRemark")[0];
                    mygrid.addJSONData(data.d);
                },
                loadError: function (xhr, status, error) {
                    var mygrid = jQuery("#listSpoRemark")[0];
                    ...
                    ErrorHandling(xhr, "spo remark grid");
                },
                loadui: 'block', //Loading message options
                autowidth: true,
                height: '100%',
                ...
                colModel: [
                           { name: 'order_no', sortable: false, hidden: true },
                           { name: 'spo_remark', label: 'Remarks', sortable: false, align: 'left',
                               cellattr: function (rowId, tv, rawObject, cm, rdata) { return 'style="white-space: normal !important;"' }
                           }
                          ],
                jsonReader: {
            ...
            //Hide Grid column headers
            $("#gview_listSpoRemark > .ui-jqgrid-hdiv").hide();
        });
